...
const PATCH_SENTINEL = "/*codex-linux:shared-app-server-socket:v2*/";
const ATTACHMENT_CLASS = "class CodexLinuxExternalAppServerSocketTransport";
const SHARED_CLASS = "class CodexLinuxSharedAppServerSocketTransport";
...
function attachmentTransportClassSource(symbols) {
  return (
    ...
    "kind=`websocket`;proxyStreams=new Set;proxyChildren=new Set;proxyKillTimers=new Map;disposed=!1;" +
    "constructor(e){this.socketPath=e}" +
    "supportsReconnect(){return!0}" +
    "validateEndpoint(){let e=require(`node:fs`),t=require(`node:path`),n=t.dirname(this.socketPath),r,i,a,o;if(!t.isAbsolute(this.socketPath))throw Error(`external app-server socket requires an absolute path`);if(n!==t.normalize(n))throw Error(`external app-server socket parent path is not canonical`);if(typeof process.getuid!=`function`)throw Error(`external app-server socket current UID is unavailable`);try{o=process.getuid()}catch(e){throw Error(`external app-server socket current UID is unavailable`)}if(!Number.isInteger(o)||o<0)throw Error(`external app-server socket current UID is unavailable`);try{r=e.lstatSync(n)}catch(e){throw Error(`external app-server socket parent inspection failed: ${e?.code??e?.message??e}`)}if(!r.isDirectory())throw Error(`external app-server socket parent is not a real directory: ${n}`);if(r.uid!==o)throw Error(`external app-server socket parent has unexpected owner`);if((r.mode&18)!==0)throw Error(`external app-server socket parent has unsafe permissions`);try{i=e.realpathSync(n)}catch(e){throw Error(`external app-server socket parent canonicalization failed: ${e?.code??e?.message??e}`)}if(i!==n)throw Error(`external app-server socket parent path contains a symlink`);try{a=e.lstatSync(this.socketPath)}catch(e){throw Error(`external app-server socket inspection failed: ${e?.code??e?.message??e}`)}if(!a.isSocket())throw Error(`external app-server endpoint is not a real Unix socket: ${this.socketPath}`);if(a.uid!==o)throw Error(`external app-server socket has unexpected owner`);if((a.mode&384)!==384)throw Error(`external app-server socket owner read and write permissions are required`);if((a.mode&63)!==0)throw Error(`external app-server socket has unsafe group or other permissions`)}" +
    ...
    "dispose(){this.disposed=!0;for(let e of this.proxyStreams)e.destroy();this.proxyStreams.clear();for(let e of this.proxyChildren)this.stopProxy(e)}" +
    "createProxyStream(){if(this.disposed)throw Error(`external app-server socket transport is disposed`);let c=process.env.CODEX_CLI_PATH;if(!c)throw Error(`external app-server socket requires CODEX_CLI_PATH`);let e=require(`node:child_process`).spawn(c,[`app-server`,`proxy`,`--sock`,this.socketPath],{env:process.env,stdio:[`pipe`,`pipe`,`pipe`]}),t=e.stdin,n=e.stdout,r=e.stderr;this.trackProxy(e);if(t==null||n==null||r==null)throw this.stopProxy(e),Error(`external app-server proxy stdio was unavailable`);let i=``;r.on(`data`,e=>{i=`${i}${e.toString(`utf8`)}`.slice(-4000)});let s=this,a=new(require(`node:stream`).Duplex)({read(){n.resume()},write(e,n,r){t.write(e,n,r)},final(e){t.end(),e()},destroy(t,n){s.stopProxy(e),n(t)}});Object.assign(a,{setKeepAlive:()=>a,setNoDelay:()=>a,setTimeout:()=>a});let o=e=>a.destroy(e);t.on(`error`,o),n.on(`data`,e=>{a.push(e)||n.pause()}),n.on(`end`,()=>a.push(null)),e.on(`error`,o),e.on(`close`,(e,n)=>{t.removeListener(`error`,o),e===0?a.push(null):a.destroy(Error(`external app-server proxy exited (${e??n??`unknown`}): ${i.trim()}`))}),this.proxyStreams.add(a),a.once(`close`,()=>this.proxyStreams.delete(a));return a}" +
    `async connect(){if(this.disposed)throw Error(\`external app-server socket transport is disposed\`);this.validateEndpoint();let e={current:null},t;try{t=new ${symbols.namespace}.${symbols.webSocketClass}(${symbols.webSocketUrl},{perMessageDeflate:!1,createConnection:()=>(e.current=this.createProxyStream(),e.current)}),t.once(\`close\`,()=>e.current?.destroy()),await new Promise((n,r)=>{let i=setTimeout(()=>o(Error(\`external app-server websocket open timed out\`)),3e4);i.unref();let a=()=>{clearTimeout(i),t.off(\`error\`,o),t.off(\`close\`,s)},o=e=>{a(),r(e)},s=()=>o(Error(\`external app-server websocket closed before opening\`));t.once(\`open\`,()=>{a(),n()}),t.once(\`error\`,o),t.once(\`close\`,s)}),${symbols.namespace}.${symbols.keepAlive}(t,{onPongTimeout:()=>t.terminate()});return new ${symbols.namespace}.${symbols.adapterClass}(t)}catch(n){e.current?.destroy(),t?.terminate(),await new Promise(e=>setTimeout(e,0));throw n}}}`
  );
}
...
function applySharedAppServerSocketPatch(source) {
  ...
  let patchedFactory = factorySource.replace(
    localFallbackPattern,
    (match) => `${match}${SHARED_SELECTOR}`,
  );
